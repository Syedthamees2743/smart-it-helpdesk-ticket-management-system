...
const DepartmentTable = ({ departments, loading, onEdit, onToggleStatus, onDelete }) => {
  if (loading) {
    return (
      <div className="text-center py-5">
        ...
        <br/><span className="text-muted mt-2 d-block">Loading departments...</span>
      </div>
    );
  }
  ...
  // Badge color based on department_type
  const getTypeBadge = (type) => {
    switch (type) {
      case 'employee':
        return <Badge bg="primary" className="me-2">EMP</Badge>;
      case 'technician':
        return <Badge bg="info" className="me-2">TECH</Badge>;
      case 'both':
        return <Badge bg="success" className="me-2">ALL</Badge>;
      default:
        return null;
    }
  };
      ...
      <tbody>
        {departments.map(dept => (
          <tr key={dept.id}>
            ...
            <td>{getTypeBadge(dept.department_type)}</td>
            <td className="text-muted">
              {dept.description 
                ? (dept.description.length > 40 
                    ? dept.description.substring(0, 40) + '...' 
                    : dept.description) 
                : '-'
              }
            </td>
