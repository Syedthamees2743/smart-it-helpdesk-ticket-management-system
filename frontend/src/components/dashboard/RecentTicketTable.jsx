...
const RecentTicketsTable = ({ tickets, employeeView = false }) => {
  const navigate = useNavigate();

  if (!tickets || tickets.length === 0) {
    return <div className="text-center text-muted py-4">No recent tickets found.</div>;
  }
      ...
      <tbody>
        {tickets.map((t, idx) => (
          <tr key={idx}>
            ...
            <td><Badge className={`badge-priority-${t.priority.toLowerCase()}`}>{t.priority}</Badge></td>
            {!employeeView && <td className="text-muted">{t.tech}</td>}
            <td><Badge className={`badge-status-${t.status.toLowerCase().replace(' ', '_')}`}>{t.status}</Badge></td>
            <td>
              <span className={`fw-bold ${t.sla === 'Breached' ? 'text-danger' : 'text-success'}`}>
                {t.sla}
              </span>
            </td>
            ...
            <td>
              ...
              <Button variant="outline-primary" size="sm" onClick={() => navigate(`${t.id}`)}>
