...
// ======================================================
// FEEDBACK
// IMPORTANT: default import, NOT named import
// ======================================================
...
const TechnicianTicketDetails = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setComments(
        commentRes.data?.results || commentRes.data || []
      );
    } catch (err) {
  ...
  //
  // Feedback is shown only when ticket is CLOSED
  // ======================================================
  ...
  useEffect(() => {
    ...
    const fetchFeedback = async () => {
      ...
      try {
        // IMPORTANT:
        // feedbackService uses default export
        const res = await feedbackService.getTicketFeedback(id);

        // getTicketFeedback() already returns response.data
  ...
  const handleAiTroubleshoot = async () => {
    ...
    try {
      const res = await aiService.troubleshootTicket({
      ...
      if (err.response?.status === 403) {
        setAiError(
          'You can only analyze tickets assigned to you.'
        );
        ...
        setAiError(
          'AI assistance is currently unavailable. You can continue troubleshooting manually.'
        );
      }
    } finally {
      setAiLoading(false);
  ...
  const getPriorityBadge = (priority) => {
    ...
    if (priority === 'low') bg = 'info';
    else if (priority === 'medium') bg = 'primary';
    else if (priority === 'high') bg = 'warning';
    else if (priority === 'critical') bg = 'danger';
  ...
  const renderActions = () => {
    ...
    if (ticket.status === 'resolved') {
      return (
        <div
          className="d-flex align-items-center gap-3 p-4 rounded-4 border mb-4"
          style={{
        ...
        >
          ...
          <div>
            ...
            <div className="text-muted small">
              This ticket is marked as resolved. The employee
              will review and either close it or reopen it.
            </div>
          </div>
        ...
        <Col lg={8}>
          ...
          {feedbackLoading &&
            ticket.status === 'closed' && (
              <div className="d-flex align-items-center justify-content-center gap-2 py-3 mb-4">
          ...
          {aiError && (
            <Alert
              variant="warning"
              className="d-flex align-items-start rounded-4 border-0 py-3 mb-4"
              dismissible
              onClose={() => setAiError('')}
            >
          ...
          {aiResult && (
            <Card className="border-0 shadow-sm rounded-4 mb-4 overflow-hidden">
              ...
              >
                ...
                <div className="d-flex align-items-center gap-2">
                  ...
                  <div>
                    ...
                    >
                      Smart analysis for this ticket
                    </span>
                  </div>
              ...
              <Card.Body className="p-4">
                  ...
                  aiResult.possible_causes.length > 0 && (
                    <div className="mb-4">
                      ...
                      <div className="ps-4 d-flex flex-column gap-2">
                        ...
                        {aiResult.possible_causes.map(
                          (cause, index) => (
                            <div
                              key={index}
                              className="d-flex align-items-start p-2 rounded-3"
                              style={{
                ...
                {aiResult.troubleshooting_steps &&
                  aiResult.troubleshooting_steps.length > 0 && (
                    <div className="mb-4">
                      ...
                      <div className="ps-4">
                        ...
                        {aiResult.troubleshooting_steps.map(
                          (step, index) => (
                            <div
                              key={index}
              ...
              {comments.length === 0 ? (
                ...
                >
                  ...
                  {comments.map((comment) => (
                    <div
                      key={comment.id}
                      className="mb-3 p-3 bg-light rounded-4 border"
                    >
        ...
        <Col lg={4}>
          ...
          <Card className="border-0 shadow-sm rounded-4">
            ...
            <Card.Body className="p-4">
              ...
              <h6 className="fw-bold border-bottom pb-3 mb-4 text-dark">
                Status Timeline
              </h6>
