...
const MyTickets = () => {
    ...
    useEffect(() => {
        getCategories().then(res => setCategories(res.data.results || res.data)).catch(err => {});
    }, []);
    ...
    useEffect(() => {
        let params = { search, status: statusFilter, priority: priorityFilter, category: categoryFilter };
        fetchData(params);
        ...
        <div>
            <div className="d-flex justify-content-between align-items-center mb-4">
                <div>
                    ...
                    <p className="text-muted mb-0">Track and manage your IT support requests.</p>
                </div>
                <Button variant="primary" onClick={() => navigate('/employee/tickets/new')}>
            ...
            <Card className="border-0 shadow-sm">
                <Card.Body>
                    <Row className="g-2 mb-3">
                        <Col md={4}><Form.Control placeholder="Search tickets..." value={search} onChange={(e) => setSearch(e.target.value)} /></Col>
                        <Col md={2}><Form.Select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}><option value="">All Status</option><option value="open">Open</option><option value="assigned">Assigned</option><option value="in_progress">In Progress</option><option value="resolved">Resolved</option><option value="closed">Closed</option></Form.Select></Col>
                    ...
                    tickets.length === 0 ? <div className="text-center py-5 text-muted"><h5>No tickets found.</h5><p>You haven't raised any support requests yet.</p></div> : (
                        <Table hover responsive className="align-middle" style={{fontSize:'0.9rem'}}>
                            <thead className="table-light"><tr><th>Ticket #</th><th>Title</th><th>Category</th><th>Priority</th><th>Technician</th><th>Status</th><th>SLA</th><th>Action</th></tr></thead>
                            <tbody>
                                {tickets.map(t => (
                                    <tr key={t.id}>
                                        ...
                                        <td><Button size="sm" variant="outline-primary" onClick={() => navigate(`/employee/tickets/${t.id}`)}>View</Button></td>
                                    </tr>
                                ))}
                            </tbody>
                        </Table>
