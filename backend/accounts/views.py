...
class RegisterView(generics.CreateAPIView):
    ...
    # ===== CHANGED: return the backend-generated profile ID in the response.
    # Response shape stays the same (user fields), employee_id /
    # technician_id are ADDED — nothing existing breaks.
    def create(self, request, *args, **kwargs):
        ...
        if employee_profile:
            data['employee_id'] = employee_profile.employee_id

        technician_profile = getattr(user, 'technician_profile', None)
        if technician_profile:
            data['technician_id'] = technician_profile.technician_id
...
class UserManagementView(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # ===== No change needed here: UserSerializer already includes
        # employee_id / technician_id, and they are now always populated
        # (generated by EmployeeProfile.save() via the post_save signal).
...
class EmployeeSignupView(APIView):
    ...
    def post(self, request):
        try:
            ...
            # Get the auto-generated employee_id from the created profile
            try:
            ...
            # Validation errors must return 400 with field errors (not 500)
            raise
...
class TechnicianSignupView(APIView):
    ...
    def post(self, request):
        try:
            ...
            # Get the auto-generated technician_id from the created profile
            try:
    ...
    def patch(self, request, pk):
        ...
        if user.role == 'employee':
            try:
                ...
                # ===== create() triggers save() → ID generated automatically
...
def create_user_profile(sender, instance, created, **kwargs):
    ...
    ===== CHANGED: no 'employee_id': '' default anymore — the profile's
    save() generates the EMP-/TECH- ID automatically. This also fixes
    the bug where two admin-created employees both got employee_id=''
